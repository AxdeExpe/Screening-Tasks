...
class OHLCDataReader:
    ...
    def _is_unix_time(self, timestamp: int) -> bool:
        ...
        if (timestamp.bit_length() + 2) // 3 != 13:
            return False
...
class OHLCAnalyzer:
    ...
    def analyze_close_change(self, procent_diff: float = 3.0, time_interval: int = 60 * 60 * 1000) -> Iterator[Tuple[float, list]]: # 1 hour
        ...
        if procent_diff < 0.0 or procent_diff > 100.0:
            raise ValueError("Procent diff must be between 0 and 100")
        ...
        for row in self.ohlc_reader.read():
            ...
            if ohlc_pair[-1] is not None:
                ...
                price_change_procent = (ohlc_pair[0][close_price_index] - ohlc_pair[1][close_price_index]) / ohlc_pair[0][close_price_index] * 100

                if price_change_procent > procent_diff:
                    yield price_change_procent, ohlc_pair
    ...
    def show_n_first(self, n: int = 5) -> None:
        ...
        for i, (price_change_procent, ohlc_pair) in enumerate(self.analyze_close_change(procent_diff=3.0, time_interval=60 * 60 * 1000)):
            ...
            print("{:<10.2f}% {:<10} {:<10} {:<10} {:<10}".format(price_change_procent, close_time_start, ohlc_pair[0][close_price_index], close_time_end, ohlc_pair[1][close_price_index]))
